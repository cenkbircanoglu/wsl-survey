...
parser = argparse.ArgumentParser(
    description='Pytorch Cell Classifier Training')
...
assert os.path.isdir(
    'checkpoint/' +
    trainset_dir), '[Error]: No model has been trained on the dataset!'
file_name = get_network(args)
        ...
        for f in files:
            ...
            if is_image(f):
                ...
                if use_gpu:
                    inputs = inputs.cuda()
                inputs = inputs.view(
                    1, inputs.size(0), inputs.size(1),
                    inputs.size(2))  # add batch dim in the front
                ...
                if not (dset_classes[index] in file_path.split("/")[-1]):
                    print(file_path + "," + str(dset_classes[index]) + ": " +
                          str(score))
                """
