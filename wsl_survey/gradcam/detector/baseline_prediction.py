...
for thresh in [200, 1]:
    ...
    for test_num in range(1, 27 + 1):
        ...
        baseline_dir = '/home/bumsoo/Data/baseline_info/%d_TEST%d.csv' % (
            thresh, test_num)
        ...
        with open(baseline_dir, 'r') as csvfile:
            ...
            with open(
                    'results/baseline/%d/TEST%d/TEST%d.csv' %
                (thresh, test_num, test_num), 'w') as wrfile:
                fieldnames = ['prediction', 'x', 'y', 'w', 'h']
                writer = csv.DictWriter(wrfile, fieldnames=fieldnames)

                original_img = cv2.imread(background_root +
                                          'TEST%d.png' % test_num)
