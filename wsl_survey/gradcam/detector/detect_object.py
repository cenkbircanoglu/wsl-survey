...
from wsl_survey.gradcam.detector.grad_cam import *
from wsl_survey.gradcam.detector.grad_cam import GradCAM
...
def getNetwork(args):
    if (args.net_type == 'alexnet'):
        ...
        print(
            '[Error]: Network should be either [alexnet / vgget / resnet / densenet]'
        )
        sys.exit(1)
...
def generate_sliding_windows(image, stepSize, windowSize):
    ...
    for x in range(0, image.size[0], stepSize):
        for y in range(0, image.size[1], stepSize):
            if (x + windowSize <= image.size[0]
                    and y + windowSize <= image.size[1]):
                list_windows.append(
                    image.crop((x, y, x + windowSize, y + windowSize)))
            elif (x + windowSize > image.size[0]
                  and y + windowSize > image.size[1]):
                list_windows.append(
                    image.crop((image.size[0] - windowSize,
                                image.size[1] - windowSize, image.size[0],
                                image.size[1])))
            elif (x + windowSize > image.size[0]):
                list_windows.append(
                    image.crop((image.size[0] - windowSize, y, image.size[0],
                                y + windowSize)))
            elif (y + windowSize > image.size[1]):
                list_windows.append(
                    image.crop((x, image.size[1] - windowSize, x + windowSize,
                                image.size[1])))
...
def generate_padding_image(image, mode='cv2'):
    if (mode == 'cv2'):
        border_x = int((args.stepSize -
                        ((image.shape[0] - args.windowSize) % args.stepSize)))
        border_y = int((args.stepSize -
                        ((image.shape[1] - args.windowSize) % args.stepSize)))
        ...
        border_x = args.stepSize - (
            (image.size[0] - args.windowSize) % args.stepSize)
        border_y = args.stepSize - (
            (image.size[1] - args.windowSize) % args.stepSize)
        pad_image = Image.new(
            "RGB", (image.size[0] + border_x, image.size[1] + border_y),
            color=255)
...
if __name__ == "__main__":
    ...
    assert os.path.isdir(
        '../classifier/checkpoint'), '[Error]: No checkpoint directory found!'
    assert os.path.isdir(
        '../2_classifier/checkpoint/' +
        trainset_dir), '[Error]: There is no model weight to upload!'
    file_name = getNetwork(args)
    checkpoint = torch.load('../2_classifier/checkpoint/' + trainset_dir +
                            file_name + '.t7')
