...
from wsl_survey.datasets.voc2007 import dataset
from wsl_survey.wildcat.engine import MultiLabelMAPEngine
from wsl_survey.wildcat.models import resnet101_wildcat


def main_voc2007(args):
    use_gpu = torch.cuda.is_available()

    # define dataset
    train_dataset = dataset.ClassificationDataset(args.data, 'trainval')
    val_dataset = dataset.ClassificationDataset(args.data, 'test')
    num_classes = 20

    # load model
    model = resnet101_wildcat(
        num_classes,
        pretrained=True,
        kmax=args.k,
        alpha=args.alpha,
    ...
    state = {
        ...
        'save_model_path': '../expes/models/voc2007/'
    }
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        'data', metavar='DIR', help='path to dataset (e.g. ../data/')
    parser.add_argument(
        '--image-size',
        '-i',
        default=224,
