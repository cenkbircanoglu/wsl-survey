...
class PathIndex:
    ...
    def get_search_paths_dst(self, max_radius=5):
        ...
        path_list_by_length = [np.asarray(v) for v in coord_indices_by_length
                               if v]
    ...
    def get_path_indices(self, size):
        ...
        src_indices = np.reshape(full_indices[:cropped_height,
...
def affinity_sparse2dense(affinity_sparse, ind_from, ind_to, n_vertices):
    ...
    indices_id = torch.stack([torch.arange(0, n_vertices).long(),
...
def to_transition_matrix(affinity_dense, beta, times):
    ...
    trans_mat = scaled_affinity / torch.sum(scaled_affinity, dim=0,
                                            keepdim=True)
...
def propagate_to_edge(x, edge, radius=5, beta=10, exp_times=8):
    ...
    edge_padded = F.pad(edge, (radius, radius, 0, radius), mode='constant',
                        value=1.0)
