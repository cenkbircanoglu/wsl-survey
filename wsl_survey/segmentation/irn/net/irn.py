...
class SmallAffinityDisplacementLoss(SmallNet):
    path_indices_prefix = "path_indices"

    def __init__(self, path_index, backbone=None):

        super(SmallAffinityDisplacementLoss, self).__init__(backbone=backbone)
    ...
    def to_affinity(self, edge):
        ...
        for i in range(self.n_path_lengths):
            ind = self._buffers[
                SmallAffinityDisplacementLoss.path_indices_prefix + str(i)]
            ind_flat = ind.view(-1)
