...
class PathIndex:
    def __init__(self, radius, default_size):
        ...
        self.search_paths, self.search_dst = self.get_search_paths_dst(
            self.radius)

        self.path_indices, self.src_indices, self.dst_indices = self.get_path_indices(
            default_size)
    ...
    def get_search_paths_dst(self, max_radius=5):
        ...
        path_list_by_length = [
            np.asarray(v) for v in coord_indices_by_length if v
        ]
        path_destinations = np.concatenate(
            [p[:, 0] for p in path_list_by_length], axis=0)
    ...
    def get_path_indices(self, size):
        ...
        src_indices = np.reshape(
            full_indices[:cropped_height,
                         self.radius_floor:self.radius_floor + cropped_width],
            -1)
...
def edge_to_affinity(edge, paths_indices):
    ...
    for i in range(len(paths_indices)):
        ...
        paths_indices[i] = paths_indices[i]
        if use_cuda:
...
def affinity_sparse2dense(affinity_sparse, ind_from, ind_to, n_vertices):
    ...
    indices_id = torch.stack([
        torch.arange(0, n_vertices).long(),
        torch.arange(0, n_vertices).long()
    ])

    affinity_dense = torch.sparse.FloatTensor(
        torch.cat([indices, indices_id, indices_tp], dim=1),
        torch.cat([affinity_sparse,
                   torch.ones([n_vertices]), affinity_sparse])).to_dense()
    if use_cuda:
...
def to_transition_matrix(affinity_dense, beta, times):
    ...
    trans_mat = scaled_affinity / torch.sum(
        scaled_affinity, dim=0, keepdim=True)
...
def propagate_to_edge(x, edge, radius=5, beta=10, exp_times=8):
    ...
    path_index = PathIndex(radius=radius,
                           default_size=(ver_padded, hor_padded))

    edge_padded = F.pad(edge, (radius, radius, 0, radius),
                        mode='constant',
                        value=1.0)
    ...
    dense_aff = affinity_sparse2dense(sparse_aff, path_index.src_indices,
                                      path_index.dst_indices,
                                      ver_padded * hor_padded)
