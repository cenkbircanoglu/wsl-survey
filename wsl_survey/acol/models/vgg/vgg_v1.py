...
class VGG(nn.Module):
    ...
    def forward(self, x, label=None):
        ...
        feat_erase = self.erase_feature_maps(localization_map_normed, feat,
                                             self.threshold)
    ...
    def add_heatmap2img(self, img, heatmap):
        ...
        heatmap = heatmap * 255
        color_map = cv2.applyColorMap(heatmap.astype(np.uint8),
                                      cv2.COLORMAP_JET)
        img_res = cv2.addWeighted(img.astype(np.uint8), 0.5,
                                  color_map.astype(np.uint8), 0.5, 0)
    ...
    def save_erased_img(self, img_path, img_batch=None):
        ...
        if len(img_batch.size()) == 4:
            ...
            for batch_idx in range(batch_size):
                ...
                atten_map = F.upsample(self.attention.unsqueeze(dim=1),
                                       (224, 224),
                                       mode='bilinear')
    ...
    def get_atten_map(self, feature_maps, gt_labels, normalize=True):
        ...
        atten_map = torch.zeros(
            [feature_map_size[0], feature_map_size[2], feature_map_size[3]])
        if use_cuda:
            atten_map = atten_map.cuda()
        atten_map = Variable(atten_map)

        for batch_idx in range(batch_size):
            atten_map[batch_idx, :, :] = torch.squeeze(
                feature_maps[batch_idx, label.data[batch_idx], :, :])
...
cfg = {
    ...
    'D1': [
        64, 64, 'M', 128, 128, 'M', 256, 256, 256, 'M', 512, 512, 512, 'N',
        512, 512, 512, 'N'
    ],
