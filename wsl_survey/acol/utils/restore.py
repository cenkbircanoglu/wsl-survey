...
def restore(args, model, optimizer, istrain=True, including_opt=False):
    if os.path.isfile(args.restore_from) and ('.pth' in args.restore_from):
        ...
        if len(filelist) > 0:
            filelist.sort(
                key=lambda fn: os.path.getmtime(os.path.join(restore_dir, fn)),
                reverse=True)
...
def _model_load_v6(model, pretrained_dict):
    ...
    feature2_pred_b = {
        'module.fc5_seg.%d.bias' % (i): 'module.features.%d.bias' % (i + 24)
        for i in range(0, 5, 2)
    }
...
def _model_load_v2(model, pretrained_dict):
    ...
    fc5_cls_b = {
        'module.fc5_cls.%d.bias' % (i): 'module.features.%d.bias' % (i + 24)
        for i in range(0, 5, 2)
    }
    fc5_seg_w = {
        'module.fc5_seg.%d.weight' % (i):
