...
def get_mAP(gt_labels, pred_scores):
    ...
    for i in range(n_classes):
        res = metrics.average_precision_score(gt_labels[:, i], pred_scores[:,
                                                                           i])
        results.append(res)
...
def get_iou(pred, gt):
    ...
    result_class = count
    unique_classes = len(np.unique(gt)) - 1 if 255 in np.unique(
        gt).tolist() else len(np.unique(gt))
...
def fast_hist(pred, gt, n=21):
    ...
    gt = _to_numpy(gt)
    k = (gt >= 0) & (gt < n)
    return np.bincount(n * pred[k].astype(int) + gt[k],
