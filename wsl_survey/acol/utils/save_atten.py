...
class SAVE_ATTEN(object):
    ...
    def _get_idx2cate_dict(self, datasetname=None):
        if datasetname not in idx2catename.keys():
            print(
                'The given %s dataset category names are not available. The supported are: %s' \
                % (str(datasetname), ','.join(idx2catename.keys())))
            return None
        else:
            return {
                idx: cate_name
                for idx, cate_name in enumerate(idx2catename[datasetname])
            }
    ...
    def _save_masked_img(self, img_path, atten, label):
        ...
        print(np.shape(attention_map), 'Max:', np.max(attention_map), 'Min:',
              np.min(attention_map))
        ...
        heat_map = cv2.applyColorMap(atten_norm.astype(np.uint8),
                                     cv2.COLORMAP_JET)
        img = cv2.addWeighted(img.astype(np.uint8), 0.5,
                              heat_map.astype(np.uint8), 0.5, 0)
    ...
    def save_top_5_atten_maps(self,
                              atten_fuse_batch,
                              top_indices_batch,
                              org_paths,
                              topk=5):
        ...
        for idx in range(img_num):
            ...
            for k in range(topk):
                ...
                save_path = os.path.join(self.save_dir, 'heat_maps',
                                         'top%d' % (k + 1))
    ...
    def _add_msk2img(self, img, msk, isnorm=True):
        ...
        heat_map = cv2.applyColorMap(atten_norm.astype(np.uint8),
                                     cv2.COLORMAP_JET)
        w_img = cv2.addWeighted(img.astype(np.uint8), 0.5,
                                heat_map.astype(np.uint8), 0.5, 0)
    ...
    def _draw_text(self, pic, txt, pos='topleft'):
        ...
        stat_y = 30
        for t in txt:
            pic = cv2.putText(pic, t, (10, stat_y), font, 0.8, (255, 255, 255),
                              2, cv2.LINE_AA)
            stat_y += 30
    ...
    def get_heatmap_idxes(self, gt_label):
        ...
        if np.ndim(gt_label) == 1:
            ...
            for row in gt_label:
                idxes = np.where(row[0] == 1)[0] if np.ndim(row) == 2 else \
                    np.where(row == 1)[0]
        ...
        if isinstance(img_path, list) or isinstance(img_path, tuple):
            ...
            for i in range(batch_size):
                ...
                for k in label_indexes[i]:
                    ...
                    suffix = str(k)
                    if only_map:
                        save_img = (self.normalize_map(atten_map_k) *
                                    255).astype(np.int)
                    else:
                        save_img = msked_img
                    ...
                    if maps_in_dir:
                        cv2.imwrite(
                            os.path.join(img_save_dir, suffix + '.png'),
                            save_img)
                    else:
                        cv2.imwrite(
                            os.path.join(save_dir,
                                         img_name + '_' + suffix + '.png'),
                            save_img)
