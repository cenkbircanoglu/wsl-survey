...
for eps in range(2, 50, 3):
    for min_sample in [2, 3, 4, 8, 16, 32]:
        cls = cluster.DBSCAN(eps=eps, leaf_size=50, n_jobs=-1, min_samples=min_sample)
        cls = cls.fit(X)
        labels = cls.labels_
        label_d = dict()
        ...
        np.save('./data/voc12/cls_dbscan%s_%s_labels.npy' % (eps, min_sample), label_d)

        with open('./data/voc12/category_size.txt', mode='a') as f:
            f.write('%s=%s %s %s\n' % ('dbscan_eps', eps, min_sample, category_size))
